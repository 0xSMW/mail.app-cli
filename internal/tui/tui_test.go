...
func loadedModel(t *testing.T) model {
	t.Helper()
	m := newModel(mail.NewClient(), Options{})
	...
	next, _ = m.Update(messagesLoadedMsg{requestResult: requestResult{requestID: m.listLane.id}, messages: messages, source: m.sidebar.current()})
	return next.(model)
}
